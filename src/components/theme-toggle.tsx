...
const ThemeToggle = ({
  ...
  // Initialize theme based on server-side data-theme and cookie
  useEffect(() => {
    const html = document.documentElement;
    const themeCookie =
      getCookie("theme") || html.getAttribute("data-theme") || "system";
    const isDark =
      themeCookie === "dark" ||
      (themeCookie === "system" &&
    ...
    const handleChange = () => {
      if (theme === "system") {
        ...
        document.cookie = `theme=${
          prefersDark ? "dark" : "light"
        }; path=/; max-age=${60 * 60 * 24 * 3650}`; // Update cookie
        setTheme(theme);
      }
    };
    if (theme === "system") handleChange(); // Initial sync
    mediaQuery.addEventListener("change", handleChange);
    return () => mediaQuery.removeEventListener("change", handleChange);
  }, [theme]);

  // Handle theme toggle and update cookie
  const toggleTheme = () => {
    const newTheme =
      theme === "dark" ? "light" : theme === "light" ? "system" : "dark";
    setTheme(newTheme);
    document.cookie = `theme-name=${newTheme}; path=/; max-age=${
      60 * 60 * 24 * 3650
    }`; // 3650 days
    ...
    if (newTheme === "system") {
      ...
      document.cookie = `theme=${
        prefersDark ? "dark" : "light"
      }; path=/; max-age=${60 * 60 * 24 * 3650}`; // 3650 days
    } else {
